...
- VLM 多模态：通过 HTTP 转发到独立的 Python `vlm_api`（可部署在另一台 GPU 服务器上，Go 后端运行在另一台服务器上，并通过 SSH 反向隧道访问）
...
- `cliptest/`：模板灵感跟拍功能，基于 CLIP 多模态模型对输入图片提取特征，结合 FAISS 索引进行向量相似度检索，返回最相似的模板封面供上层 FastAPI 服务调用。该层只关心特征提取与向量检索，不涉及业务逻辑与数据库。
- `static/`：静态资源（上传图片、模板等），运行时通过 `/static` 暴露
- `deploy/systemd/`：systemd 示例配置
- `qwen-vl/`：GPU服务器，Python推理微服务,qwen2.5-Instruct-7B模型推理输出拍摄建议与姿势引导
...
## VLM（GPU 服务器）联调方式（推荐：SSH 反向隧道）

典型部署是：

- 后端服务器（无显卡）：运行本 Go 后端
- GPU 服务器：运行 `vlm_api`（仅监听本机 `127.0.0.1:8000`）
- 通过 SSH 反向隧道把 GPU 服务器的 `127.0.0.1:8000` 映射到后端服务器的 `127.0.0.1:<端口>`

### 1）在 GPU 服务器启动 `vlm_api`
